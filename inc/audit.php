...
function identity_security_kit_render_audit_page() {
	...
	<div class="wrap identity-security-audit-admin">
		...
		<?php if ( $total_pages > 1 ) : ?><nav class="isk-admin-pagination" aria-label="<?php esc_attr_e( 'Security audit pagination', 'identity-security-kit' ); ?>"><?php echo wp_kses_post( paginate_links( array( 'base' => add_query_arg( array_merge( $base_args, array( 'paged' => '%#%' ) ), admin_url( 'admin.php' ) ), 'current' => $args['paged'], 'total' => $total_pages, 'type' => 'list', 'prev_text' => __( 'Previous', 'identity-security-kit' ), 'next_text' => __( 'Next', 'identity-security-kit' ) ) ) ); ?></nav><?php endif; ?>
	</div>
	<style>
		...
		.identity-security-audit-admin .isk-admin-pagination{display:flex;clear:both;justify-content:flex-end;margin:18px 0 28px}.identity-security-audit-admin .isk-admin-pagination>.page-numbers{display:flex;gap:4px;margin:0}.identity-security-audit-admin .isk-admin-pagination li{margin:0}.identity-security-audit-admin .isk-admin-pagination a,.identity-security-audit-admin .isk-admin-pagination span{display:inline-flex;min-height:32px;box-sizing:border-box;align-items:center;padding:0 10px;border:1px solid #c3c4c7;background:#fff;text-decoration:none}.identity-security-audit-admin .isk-admin-pagination .current{border-color:#2271b1;background:#2271b1;color:#fff}@media(max-width:782px){.identity-security-audit-admin .isk-admin-pagination{justify-content:flex-start;overflow-x:auto}}
	</style>
	<?php
}
