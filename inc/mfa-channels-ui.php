...
/** Render one remote factor enrollment form. */
function identity_security_kit_render_remote_factor_enrollment( $user_id, $method, $challenge_method, $challenge_id ) {
	if ( identity_security_kit_is_mfa_method_enabled( $user_id, $method ) ) {
		return '<p>' . esc_html( sprintf( __( '%s is enabled.', 'identity-security-kit' ), identity_security_kit_get_mfa_method_label( $method ) ) ) . '</p>';
	}
	...
	<section class="identity-security-mfa-channels">
		...
		<?php if ( in_array( 'email', $allowed, true ) ) : ?>
			...
			<?php if ( function_exists( 'identity_security_kit_is_email_verified' ) && identity_security_kit_is_email_verified( $user_id ) ) : ?>
				<?php echo identity_security_kit_render_remote_factor_enrollment( $user_id, 'email', $challenge_method, $challenge_id ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
		...
		<?php if ( in_array( 'sms', $allowed, true ) ) : ?>
			...
			<?php if ( identity_security_kit_is_phone_verified( $user_id ) ) : ?>
				<?php echo identity_security_kit_render_remote_factor_enrollment( $user_id, 'sms', $challenge_method, $challenge_id ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
			<?php endif; ?>
		<?php endif; ?>
