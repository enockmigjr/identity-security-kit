...
/**
 * Verify and enable an email or SMS MFA method.
 *
 * @param int    $user_id      User ID.
 * @param string $method       email or sms.
 * @param int    $challenge_id OTP challenge ID.
 * @param string $code         Submitted OTP code.
 * @return array<string,string>|WP_Error
 */
function identity_security_kit_enable_channel_mfa( $user_id, $method, $challenge_id, $code ) {
	$user_id      = absint( $user_id );
	$method       = sanitize_key( $method );
	$challenge_id = absint( $challenge_id );
	$purpose      = 'mfa_enrollment_' . $method;
	if ( ! get_userdata( $user_id ) || ! in_array( $method, array( 'email', 'sms' ), true ) || ! in_array( $method, identity_security_kit_get_allowed_mfa_methods( $user_id ), true ) ) {
	...
	if ( ! is_array( $recovery ) || empty( $recovery ) ) {
		...
		$recovery_token = identity_security_kit_store_recovery_display( $user_id, $codes );
		if ( is_wp_error( $recovery_token ) ) {
			$recovery_token = '';
		}
	}
