	...
	<div id="identity-security-mfa" class="identity-security-mfa">
		...
		<section class="identity-security-mfa-method" data-mfa-method="totp">
			...
			<?php elseif ( $pending_valid ) : ?>
				...
				<form method="post" action="<?php echo esc_url( identity_security_kit_get_account_action_url() ); ?>"><input type="hidden" name="action" value="identity_security_kit_totp_confirm"><?php wp_nonce_field( 'identity_security_kit_totp_confirm' ); ?><label><?php esc_html_e( 'Six-digit code', 'identity-security-kit' ); ?> <input name="otp_code" inputmode="numeric" autocomplete="one-time-code" pattern="[0-9]{6}" required></label> <button type="submit"><?php esc_html_e( 'Enable', 'identity-security-kit' ); ?></button></form>
				<form class="identity-security-mfa-secondary-action" method="post" action="<?php echo esc_url( identity_security_kit_get_account_action_url() ); ?>"><input type="hidden" name="action" value="identity_security_kit_totp_cancel"><?php wp_nonce_field( 'identity_security_kit_totp_cancel' ); ?><button type="submit"><?php esc_html_e( 'Cancel enrollment', 'identity-security-kit' ); ?></button></form>
			<?php else : ?>
				<details><summary><?php esc_html_e( 'Set up authenticator application', 'identity-security-kit' ); ?></summary><form method="post" action="<?php echo esc_url( identity_security_kit_get_account_action_url() ); ?>"><input type="hidden" name="action" value="identity_security_kit_totp_start"><?php wp_nonce_field( 'identity_security_kit_totp_start' ); ?><label><?php esc_html_e( 'Current password', 'identity-security-kit' ); ?> <input type="password" name="current_password" autocomplete="current-password" required></label> <button type="submit"><?php esc_html_e( 'Continue', 'identity-security-kit' ); ?></button></form></details>
			<?php endif; ?>
			</div>
		</section>
