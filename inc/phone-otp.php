...
function identity_security_kit_render_phone_verification_panel() {
	...
	<div class="identity-security-phone-verification">
		...
		<?php else : ?>
			...
			<form method="post" action="<?php echo esc_url( identity_security_kit_get_account_action_url() ); ?>"><input type="hidden" name="action" value="identity_security_kit_phone_otp_request"><?php wp_nonce_field( 'identity_security_kit_phone_otp_request' ); ?><button type="submit"><?php esc_html_e( 'Send phone verification code', 'identity-security-kit' ); ?></button></form>
		<?php endif; ?>
	</div>
	<?php
